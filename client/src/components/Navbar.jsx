        ...
        <div className='shadow dark:border-l-[1px]'>
            <div className='py-2 px-7 flex items-center justify-between bg-white dark:bg-slate-800'>
                ...
                <div className="relative" ref={dropdownRef}>
                    ...
                    {dropdown && (
                        <div className="dropdown z-50 absolute right-[1px] md:right-[-10px] mt-2 rounded-lg dark:bg-slate-700 border border-gray-200 w-[200px] bg-white py-2 px-1">
                            <ul className="mt-2">
                                <li className="hover:bg-gray-200 py-1 px-2 ease-linear duration-200 rounded hover:text-gray-800">
                                    <DarkModeToggle />
                                </li>
                                <li className="hover:bg-gray-200 py-1 px-2 ease-linear duration-200 rounded hover:text-gray-800">
                                    <button onClick={() => setLogoutConfirm(true)} className="w-full text-left dark:text-white dark:hover:text-gray-900">Logout</button>
                                </li>
                            </ul>
                        </div>
                    )}
            ...
            </div>
            {
                logoutConfirm && (
                    <div className='fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 z-50 flex items-center justify-center'>
                        <div className='bg-white rounded-lg shadow-lg min-w-[300px] md:w-[400px] max-w-[500px]'>
                            <div className="py-2 px-2 ">
                                <div className="flex items-center justify-end border-b pb-1">
                                    <button onClick={() => setLogoutConfirm(false)} className='bg-none text-gray-700'><X size={30} /> </button>
                                </div>
                            </div>
                            <div className="px-2">
                                <h2 className='text-xl font-[400] py-2'>Are you sure you want to logout?</h2>
                                <div className='flex gap-3 justify-end items-center py-3 px-2'>
                                    <button onClick={() => setLogoutConfirm(false)} className='bg-transparent border px-2 py-1 text-gray-800 rounded'>Cancel</button>
                                    <button onClick={logoutHandler} className='bg-red-500 px-2 text-white rounded py-1 border-transparent'>Logout</button>
                                </div>
                            </div>
                        </div>
                    </div>
